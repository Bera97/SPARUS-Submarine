...
Re = 100000; %Reynolds number, arbitrary
...
K_antenna(4,4) = int((x^2)*K_antenna(2,2), [-H2/2,H2/2]);
K_antenna(5,5) = int((x^2)*K_antenna(3,3), [-H2/2,H2/2]);
K_antenna(6,6) = Cf*2*(H2*T2+H2*W2);
K_antenna(3,5) = int(x*K_antenna(3,3),[-H2/2,H2/2]);
K_antenna(5,3) = K_antenna(3,5);
K_antenna(2,6) = int(x*K_antenna(3,3), [-H2/2,H2/2]);
...
K_USBL(4,4) = int((x^2)*K_USBL(2,2), [-H3/2,H3/2]);
K_USBL(5,5) = int((x^2)*K_USBL(3,3), [-H3/2,H3/2]);
K_USBL(6,6) = Cf*(2*pi*R3*H3);
K_USBL(3,5) = int(x*K_USBL(3,3),[-H3/2,H3/2]);
K_USBL(5,3) = K_USBL(3,5);
K_USBL(2,6) = int(x*K_USBL(3,3), [-H3/2,H3/2]);
...
K_QQ(4,4) = int((x^2)*K_QQ(2,2), [-H4/2,H4/2]);
K_QQ(5,5) = int((x^2)*K_QQ(3,3), [-H4/2,H4/2]);
K_QQ(6,6) = Cf*(2*pi*R4*H4);
K_QQ(3,5) = int(x*K_QQ(3,3),[-H4/2,H4/2]);
K_QQ(5,3) = K_QQ(3,5);
K_QQ(2,6) = int(x*K_QQ(3,3), [-H4/2,H4/2]);
K_QQ(6,2) = K_QQ(2,6);
K_QQ(3,3) = 0;
K_QQ = -K_QQ;
